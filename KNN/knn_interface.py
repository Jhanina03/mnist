...
class LiveKNNViewer:
    ...
    def _manual_pca_2d(self, x_train, x_test, max_plot=800):
        ...
        if len(x_train) > max_plot:
            ...
            train_subset_indices = rng.choice(
                len(x_train), size=max_plot, replace=False
            )
        else:
    ...
    def _build_window(self):
        ...
        tk.Label(
            right_frame,
            text="Reporte de Cálculos (Caja Blanca)",
            font=("Arial", 10, "bold"),
        ).pack(pady=5)

        scroll = tk.Scrollbar(right_frame)
        scroll.pack(side="right", fill="y")
        self.dist_text = tk.Text(
            right_frame,
            width=46,
            height=28,
            state="disabled",
        ...
        tk.Button(controls, text="◀ Anterior", command=self.previous_case).pack(
            side="left", padx=5
        )
        tk.Button(controls, text="Siguiente ▶", command=self.next_case).pack(
            side="left", padx=5
        )
    ...
    def draw_case(self, case_index):
        ...
        self._draw_digit(
            self.current_index, result["prediction"], real_label, result["confidence"]
        )
    ...
    def _update_distances_text(self, query, distances):
        ...
        if (
            hasattr(self.model, "k_validation_scores")
            and self.model.k_validation_scores
        ):
            self.dist_text.insert(
                tk.END, "HISTORIAL DE ENTRENAMIENTO (Selección de K):\n"
            )
            for k_cand, score in self.model.k_validation_scores.items():
                marker = " (K Elegido)" if k_cand == self.model.k else ""
                self.dist_text.insert(
                    tk.END, f" K={k_cand:<2} : {score * 100:.2f}%{marker}\n"
                )
        ...
        self.dist_text.insert(
            tk.END, f"{'Top':<4} | {'Dígito':<6} | {'Distancia':<9}\n"
        )
        self.dist_text.insert(tk.END, "-" * 32 + "\n")
        ...
        for rank in range(30):
            ...
            self.dist_text.insert(
                tk.END, f"{rank + 1:<4} | {lbl:<6} | {dist:.2f}{marker}\n"
            )
    ...
    def _draw_map(self, case_index, neighbor_indices, prediction, real_label):
        ...
        for digit in range(10):
            ...
            self.map_ax.scatter(
                self.train_2d[plot_indices, 0],
                self.train_2d[plot_indices, 1],
                s=10,
                color=colors[digit],
                alpha=0.3,
                label=str(digit),
        ...
        for pt, lbl in zip(neighbor_points, neighbor_labels):
            self.map_ax.scatter(
                pt[0], pt[1], s=50, color=colors[lbl], edgecolors="black", zorder=5
            )
        ...
        if len(neighbor_points) > 0:
            ...
            self.map_ax.plot(
                query_point[0] + radius * np.cos(theta),
                query_point[1] + radius * np.sin(theta),
                "k--",
                alpha=0.5,
    ...
    def _draw_digit(self, case_index, prediction, real_label, confidence):
        ...
        self.img_ax.set_title(
            f"Predicho: {prediction} ({confidence:.0%})",
    ...
    def _draw_probabilities(self, probabilities):
        ...
        self.prob_ax.bar(
            ...
            probabilities,
            color=[
                "green" if p == probabilities.max() else "gray" for p in probabilities
            ],
        )
    ...
    def show_confusion_matrix(self):
        ...
        if self.matrix is None:
            ...
            self.root.update()
            print(
                "Calculando matriz de confusión completa (10,000 tests)... esto tomará un momento."
            )
